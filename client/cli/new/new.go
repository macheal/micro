...
func copyAPIProto(c config) (string, error) {
	...
	if err != nil {
		return "", errors.New("Unable to find go-micro version. Please try `go get github.com/macheal/go-micro/v2`")
	}
	newestDir := ""
	...
	input, err := ioutil.ReadFile(fmt.Sprintf("%s/pkg/mod/github.com/macheal/go-micro/%s/api/proto/api.proto", basedir, newestDir))
	if err != nil {
		return "", err
	}
